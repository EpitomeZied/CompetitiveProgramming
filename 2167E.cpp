...
auto Solve(const int &n) -> void {
    ...
    auto check = [&](ll md) {
        int have = max(0LL, a[0] - md + 1);
        for (int i = 1; i < n; i++) {
            int gap = a[i] - a[i - 1];
            if (gap < 2 * md) continue;
            have += gap - 2 * md + 1;
        }
        have += max(0LL, x - (a[n - 1] + md) + 1);
        return (have >= k);
    };
    ...
    for (int i = 0; i <= a[0] - l && pos.size() < k; i++) {
        pos.push_back(i);
    }

    for (int i = 1; i < n && pos.size() < k; i++) {
        int s = a[i - 1] + l, e = a[i] - l;
        while (s <= e && pos.size() < k) pos.push_back(s++);
    }

    int gap = a[n - 1] + l;
    while (gap <= x && pos.size() < k) pos.push_back(gap++);
