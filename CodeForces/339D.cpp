...
int nodes[N], level[N], n = 200001;
vi arr;
int merge(const int &a, const int &b, const int &ni) {
    if (level[ni] & 1)
        return a ^ b;
    return a | b;
}
...
void build(int ni = 0,int ns = 0,int ne = n - 1) {
    ...
    build(r, m + 1, ne);
    level[ni] = level[l] + 1;
    nodes[ni] = merge(nodes[l], nodes[r] , ni);
}
...
int query(int qs,int qe,int ni = 0,int ns = 0,int ne = n - 1) {
    if (qs > ne || qe < ns) return -1; // All Out ( Put thing Doesn't Change)
    if (qs <= ns && qe >= ne)return nodes[ni];
    int l = ni * 2 + 1, r = l + 1, m = ns + (ne - ns) / 2;
    return merge(query(qs, qe, l, ns, m), query(qs, qe, r, m + 1, ne) , ni);
}
