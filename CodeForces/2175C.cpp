...
auto Solve(const int &x) -> void {
    ...
    bool flag = true;
    for (int i = 0; i < 26; i++) if (cs[i] > ct[i]) flag = false;
    ...
    vector<array<int, 26> > suff(sz(s) + 1);
    for (int k = 0; k < 26; ++k) suff[sz(s)][k] = 0;
