...
const int N = 1 << 22, M = 1'000'000'007, OO = 0X3F3F3F3F3F3F3F3F;
...
node merge(const node &a, const node &b) {
    return {
        a.open + b.open - min(a.open, b.close),
        a.close + b.close - min(a.open, b.close),
        a.ans + b.ans + min(a.open, b.close)
    };
}
...
node query(int qs,int qe,int ni = 0,int ns = 0,int ne = n - 1) {
    if (qs > ne || qe < ns) return {0, 0 , 0}; // All Out ( Put thing Doesn't Change)
