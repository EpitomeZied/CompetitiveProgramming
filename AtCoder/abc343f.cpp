...
void build(int ni = 0,int ns = 0,int ne = n - 1) {
    ...
    if (ns == ne) {
        // Leaf Node
        nodes[ni] = nodes[ni] = {arr[ns], 1, LLONG_MIN, 0};
        return;
    }
    // internal Node
...
node query(int qs,int qe,int ni = 0,int ns = 0,int ne = n - 1) {
    if (qs > ne || qe < ns) return {LLONG_MIN, 0, LLONG_MIN, 0}; // All Out ( Put thing Doesn't Change)
