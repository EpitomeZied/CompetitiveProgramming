...
struct Query {
    ...
    Query(int s, int e, int idx) : s(s), e(e), idx(idx), ans(0) {
    };
...
auto Solve(const int &n) -> void {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        seen[arr[i]] = i;
        update(i, 1);
        while (cur < m && q[cur].e == i) {
            q[cur].ans = query(q[cur].s, q[cur].e);
            cur++;
        }
    }
    sort(all(q), [&](const Query l, const Query r) -> bool {
        return l.idx < r.idx;
    });
    for (int i = 0; i < m; ++i) {
        cout << q[i].ans << endl;
