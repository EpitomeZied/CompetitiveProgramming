...
auto Solve(const int &m) -> void {
    vector<vector<pii> > grid(128, vector<pii>()); // All Digits a -> Z
    ...
    auto dist = sp('Z');
    int mn = min_element(dist.begin()+ 'A' , dist.begin() + 'Z') - dist.begin();
    cout << char(mn) << " " << dist[mn];
}
